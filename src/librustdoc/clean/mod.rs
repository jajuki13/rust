    ...
    match (trait_did, cx.tcx_opt()) {
        ...
        (Some(did), Some(ref tcx)) if tcx.lang_items.fn_trait_kind(did).is_some() => {
            ...
            let inputs = match types[0].sty {
                sty::TyTuple(ref tys) => tys.iter().map(|t| t.clean(cx)).collect(),
                _ => {
            ...
            // match types[1].sty {
            //     sty::TyTuple(ref v) if v.is_empty() => None, // -> ()
...
impl<'tcx> Clean<TyParamBound> for ty::TraitRef<'tcx> {
    fn clean(&self, cx: &DocContext) -> TyParamBound {
        ...
        for &ty_s in self.substs.types.get_slice(ParamSpace::TypeSpace) {
            ...
            if let sty::TyTuple(ref ts) = ty_s.sty {
                for &ty_s in ts {
                    if let sty::TyRef(ref reg, _) = ty_s.sty {
...
impl<'tcx> Clean<Type> for ty::Ty<'tcx> {
    fn clean(&self, cx: &DocContext) -> Type {
        match self.sty {
            ...
            ty::TyTrait(box ty::TyTrait { ref principal, ref bounds }) => {
